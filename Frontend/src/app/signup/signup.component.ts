...
errorList: ErrorDTO[]=[];
  ...
  FileSelectionChanged = (event) =>{
    if(event && event.currentTarget.files.length > 0){
    this.userImage = event.currentTarget.files[0];
    }else{
    this.userImage = null;
    }
    }
    ...
    form.append('file', this.userImage);
    form.append('NewUser', JSON.stringify(this.newUser));
    
    this._http.registerUser( form)
    .subscribe((data:any) => {
      this.errorList=data;
    ...
    showMultiple() {
      ...
      this.msgs.push({severity:'error', summary:error.PropertyName, detail:error.ErrorMessage})
      })
      } 
